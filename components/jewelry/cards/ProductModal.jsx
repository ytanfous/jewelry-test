...
const ProductModal = ({ open, onClose, session, id }) => {
    const [codes, setCodes] = useState([{ code: '', error: '' }]);
    const [showConfirmationModal, setShowConfirmationModal] = useState(false);
    const [isLoading, setIsLoading] = useState(false);
    const inputRefs = useRef([]);

    // Synchronize refs with the length of codes
    useEffect(() => {
    ...
    const handleModalSubmit = async () => {
        try {
            setIsLoading(true);
            const response = await fetch('/api/jewelers/listProducts', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
        ...
        <>
            <Modal open={open && !showConfirmationModal} onClose={onClose} >
                <div className="p-4">
                    ...
                    <h2 className="text-xl font-semibold mb-4">Entrez les codes des produits à ajouter</h2>
