...
const PdfGenerator = ({formData, handleClosePdf,updatedClientCode}) => {
    ...
    useEffect(() => {
        const total = formData.rows.reduce((sum, row) => {
            const numericValue = parseFloat(row.amount.replace(/[^\d.-]/g, ""));
                                    ...
                                    <tbody class="divide-y divide-gray-300">
                                        ${currentRows.map((row, index) => `
                                            <tr key=${row.id} class="divide-x text-center divide-gray-300">
                                                ...
                                                <td class=" whitespace-nowrap">
                                                    <p class="text-[12px] text-gray-800 mb-1">${row.amount}</p>
                                                </td>
                                                <td class="whitespace-nowrap">
                                                    <p class="text-[12px] text-gray-800 mb-1">${formatDate(row.timestamp)}</p>
                                                </td>
                                            </tr>
                                        `).join('')}
                                    </tbody>
                    ...
                    <div className="w-full text-[12px]">
                        <div className="w-full mx-auto">
                                        ...
                                        {formData.rows.map((row, index) => (
                                            <tr key={row.id} className="divide-x text-center divide-gray-300">
                                                ...
                                                {row.action === "update" ?                      <td className="px-1 py-1 whitespace-nowrap">
                                                        <p className="text-[12px] text-gray-800 pb-1">{row.amount}</p>
                                                    </td> :
                                                    <td className="px-1 py-1 whitespace-nowrap">
                                                        <p className="text-[12px] text-gray-800 pb-1">{row.amount}</p>
                                                    </td>}
                                                ...
                                                <td className="px-1 py-1 whitespace-nowrap">
                                                    <p className="text-[12px] text-gray-800 pb-1">{formatDate(row.timestamp)}</p>
                                                </td>
                                            </tr>
                                        ))}
                                        </tbody>
                            ...
                            {price && <div className="mt-2 flex flex-col justify-end">
                                ...
                                <p className="text-[12px] mt-2 text-gray-800">Le montant cumulé est de {numberAsWord} Dinar Tunisien.</p>
                            </div>}
                            <div className="mt-2">
