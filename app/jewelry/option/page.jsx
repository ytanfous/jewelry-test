    ...
    useEffect(() => {
        async function fetchElements() {
            try {
                ...
                const res = await fetch(`/api/features/getCaratByUserId?userId=${session?.user?.id}`);
                if (res.ok) {
        ...
        <>
            ...
            <div className="w-full max-w-full px-3 mb-6 sm:w-1/2 sm:flex-none xl:mb-0 xl:w-1/3">
                <div className="relative flex flex-col min-w-0 break-words shadow-md bg-white rounded-2xl bg-clip-border transition-transform duration-300 ease-in-out min-h-[200px] p-4">
                 <h3 className="font-semibold mb-4 text-2xl">List des Modèles</h3>
                    ...
                    <div className="flex gap-2 mt-4 align-middle">
                        ...
                        <button
                            onClick={addElement}
                            className="rounded-xl bg-blue-500  px-5 py-3 text-base font-medium text-white transition duration-200 hover:shadow-lg hover:shadow-blueSecondary/50 "
                        >
                            Ajouter
                        </button>
                    </div>
