...
const PdfGenerator = ({formData}) => {
    ...
    const numberAsWord = useNumberToFrenchWords(parseInt(formData?.order?.price));
    const numberAsWordAdvance = useNumberToFrenchWords(parseInt(formData?.order.advance));
    ...
    const generatePDF = async () => {
        ...
        const formattedOrderId = order.formattedOrderId || '';
        const formattedPhone = client.phone ? client.phone.replace(/(\d{2})(\d{3})(\d{3})/, "$1 $2 $3") : '';
                ...
                <div class="w-full mx-auto">
                    <div class="flex flex-col bg-white rounded-xl">
                        <div class="flex justify-between">
                            ...
                            <div class="text-end">
                        ${client.clientCode ? `
                            <h2 class="text-xl font-semibold text-gray-800">Code Client
                                # <span class="mt-1 text-gray-500">${client.clientCode}</span></h2>
                        ` : ''}
                                <h2 class="text-xl font-semibold text-gray-800">Garantie
                                    # <span class="mt-1 text-gray-500">${formattedOrderId}</span></h2>
                    ...
                    <div class="mt-2 grid sm:grid-cols-2 gap-1">
                        ...
                        <div>
                            <h3 class="text-[12px] font-semibold text-gray-800">Garantie pour : <span
                                class="text-[14px] font-semibold text-gray-800">${client.name || ''}</span></h3>
                            <address class="not-italic text-gray-500">
                                ${client.phone ? `<span class="font-semibold">Tél :</span> ${formattedPhone}<br>` : ''}
                            </address>
                        </div>` : ''}
                    </div>
                    <div class="mt-2">
                                ...
                                <tbody class="divide-y divide-gray-300">
                                    <tr class="divide-x text-center divide-gray-300">
                                        <td class="whitespace-nowrap">
                                            <p class="text-[12px] text-gray-800 mb-1">${order.model || ''}</p>
                                        </td>
                                        <td class="whitespace-nowrap">
                                            <p class="text-[12px] text-gray-800 mb-1">${order.carat || ''}</p>
                                        </td>
                                        <td class="whitespace-nowrap">
                                            <p class="text-[12px] text-gray-800 mb-1">${order.weight || ''}</p>
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
                            ...
                            <div class="w-full text-end space-y-1 flex justify-end">
                                <div class="grid grid-cols-2 sm:grid-cols-1 gap-1 sm:gap-1">
                                    <dl class="grid sm:grid-cols-5 gap-x-1">
                                        ...
                                        <dd class="col-span-2 text-gray-500">${order.price || '0'} DT</dd>
                                    </dl>
                                                                        <dl class="grid sm:grid-cols-5 gap-x-1">
                                        <dt class="col-span-3 font-semibold text-gray-800">Avance:</dt>
                                        <dd class="col-span-2 text-gray-500">${order.advance} DT</dd>
                                    </dl>
                                </div>
                            </div>
        ...
        `;
            ...
            try {
                ...
                if (i === pages - 1) {
                    const filename = `garantie-${formattedOrderId}.pdf`;
                    pdf.save(filename);
                }
            } catch (error) {
    ...
    const formattedPhone = formData.client?.phone?.replace(/(\d{2})(\d{3})(\d{3})/, "$1 $2 $3");
    return (
        <>
                    ...
                    <div className="w-full text-[12px]">
                        <div className="w-full mx-auto">
                            <div className="flex flex-col bg-white rounded-xl">
                                <div className="flex justify-between">
                                    ...
                                    <div className="text-end">
                                        {formData.client.clientCode && <h2 className="text-xl font-semibold text-gray-800">Code Client
                                            # <span className="mt-1 text-gray-500">{formData.client.clientCode}</span></h2>}
                                        <h2 className="text-xl font-semibold text-gray-800">Garantie
                                            # <span className="mt-1 text-gray-500">{formData.order.formattedOrderId}</span></h2>
                            ...
                            <div className="mt-2 grid sm:grid-cols-2 gap-1">
                                {(formData.client.name || formData.client.phone) && <div>
                                    <h3 className="text-[12px] font-semibold text-gray-800">Garantie pour : <span
                                        className="text-[14px] font-semibold text-gray-800">{formData.client.name}</span></h3>
                                    <address className="not-italic text-gray-500">
                                        {formData.client.phone && <><span
                                        ...
                                        <tbody className="divide-y divide-gray-300">
                                            <tr  className="divide-x text-center divide-gray-300">
                                                ...
                                                <td className="px-1 py-1 whitespace-nowrap">
                                                    <p className="text-[12px] text-gray-800 pb-1">{formData.order.model}</p>
                                                </td>
                                                <td className="px-1 py-1 whitespace-nowrap">
                                                    <p className="text-[12px] text-gray-800 pb-1">{formData.order.carat}</p>
                                                </td>
                                                <td className="px-1 py-1 whitespace-nowrap">
                                                    <p className="text-[12px] text-gray-800 pb-1">{formData.order.weight}</p>
                                                </td>
                                            </tr>
                                        </tbody>
                                    </table>
                            ...
                            {formData?.order.price && <div className="mt-2 flex flex-col justify-end">
                                <div className="w-full text-end space-y-1 flex justify-end">
                                    <div className="grid grid-cols-2 sm:grid-cols-1 gap-1 sm:gap-1">
                                        <dl className="grid sm:grid-cols-5 gap-x-1">
                                            ...
                                            <dd className="col-span-2 text-gray-500">{formData?.order?.price} DT</dd>
                                        </dl>
                                        {formData?.order.advance && <dl className="grid sm:grid-cols-5 gap-x-1">
                                            <dt className="col-span-3 font-semibold text-gray-800">Avance:</dt>
                                            <dd className="col-span-2 text-gray-500">{formData?.order?.advance} DT</dd>
                                        </dl>}
