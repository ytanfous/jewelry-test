...
function FilterableTable({columns, data}) {
    ...
    const filteredData = useMemo(() => {
        return data.filter(item => {
            const matchesSearchText = item.formattedSupplierId?.toLowerCase().includes(searchText.toLowerCase())
        ...
        <div>
            ...
            <div className="mt-4 flex justify-center gap-4 flex-wrap">
                <h1 className="font-light leading-none tracking-tight text-gray-900 text-2xl border-b-2 pb-2 mb-2">Nombre
                    total de Fournisseurs: <span
                        className="font-medium  pr-1 pl-1 text-blue-700">{suppliersCount}</span>
                </h1>
            </div>
